...
export interface SafeHttpFetchOptions {
  ...
  maxBytes?: number;
  /**
   * 代理环境策略：fail-closed（默认，本服务直连不走代理，代理环境下拒绝抓取）
...
export class SafeHttpFetchService {
  constructor(private readonly deps: { resolve?: Resolver; request?: Requester } = {}) {}

  async fetch(rawUrl: string, options: SafeHttpFetchOptions = {}): Promise<SafeFetchResult> {
    if (options.proxyPolicy !== "ignore" && hasProxyEnvironment()) throw new Error("URL 抓取在代理环境中 fail closed；请先经 WebFetch 落地本地资产");
    ...
    for (let redirect = 0; redirect <= maxRedirects; redirect += 1) {
      ...
      if (addresses.length === 0 || addresses.some((address) => !isPublicIpAddress(address.address) && !isFakeIpRange(address.address))) throw new Error("URL DNS 结果包含非公网或混合地址");
      ...
      if ([301, 302, 303, 307, 308].includes(response.status)) {
        ...
        if (redirect === maxRedirects) throw new Error("URL 重定向次数超限");
        current = normalizeUrl(new URL(location, current).toString());
        continue;
      }
      if (response.status < 200 || response.status >= 300) throw new Error(`URL 请求失败: HTTP ${response.status}`);
...
function normalizeUrl(raw: string): URL {
  ...
  if (url.protocol !== "http:" && url.protocol !== "https:") throw new Error("URL 仅允许 http/https");
  if (url.username || url.password) throw new Error("URL 不允许 credentials");
  const port = url.port ? Number(url.port) : url.protocol === "https:" ? 443 : 80;
  if ((url.protocol === "https:" && port !== 443) || (url.protocol === "http:" && port !== 80)) throw new Error("URL 仅允许标准端口");
  return url;
}
...
function requestFixedAddress(url: URL, address: LookupAddress, limits: { connectTimeoutMs: number; totalTimeoutMs: number; maxBytes: number }): Promise<{ status: number; headers: Record<string, string | string[] | undefined>; body: Uint8Array }> {
  return new Promise((resolve, reject) => {
    const request = (url.protocol === "https:" ? httpsRequest : httpRequest)({
      ...
      method: "GET",
      headers: { Host: url.host, Accept: "text/html,text/plain,application/json;q=0.8,*/*;q=0.2", "User-Agent": "Lume-Fetch/1" },
      ...
      response.on("data", (chunk: Buffer) => {
        ...
        if (size > limits.maxBytes) {
          request.destroy(new Error("URL 响应超过大小限制"));
          return;
        }
        chunks.push(chunk);
      });
    ...
    request.setTimeout(limits.connectTimeoutMs, () => request.destroy(new Error("URL 连接超时")));
    const total = setTimeout(() => request.destroy(new Error("URL 总超时")), limits.totalTimeoutMs);
    total.unref();
