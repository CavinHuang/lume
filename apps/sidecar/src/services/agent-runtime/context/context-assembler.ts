...
export class ContextAssembler {
  ...
  private async assembleWithoutContextSpan(
    ...
    const browserFallbackPolicy = hasBuiltInBrowserTools
      ? "Lume's task-owned in-app Browser is available through the mcp__browser__* tools for the whole user request, including all internal Agent iterations. Call these tools directly; do not activate browser:browser, bootstrap JavaScript bindings, or guess Node REPL APIs. Start with mcp__browser__list_tabs and reuse its locked task tab; call mcp__browser__open only when no suitable task tab exists. Call mcp__browser__snapshot before interaction, then pass its refs to click, fill, press, select, check, or scroll. Each action returns a fresh snapshot; use only refs from the newest snapshot and call snapshot again after stale_target. If a tool returns user_takeover_required, stop all Browser actions and wait for the user to explicitly return control; never retry or switch to computer-use. Use mcp__browser__run_script only when the built-in semantic tools cannot express the operation. Use native computer-use only after a Browser tool returns browser_unavailable, and state that capability was degraded."
