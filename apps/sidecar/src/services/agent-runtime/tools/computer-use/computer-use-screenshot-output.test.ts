...
describe("saveComputerUseScreenshots", () => {
  ...
  test("preserves the host v3 screenshot id used for action cache validation", () => {
    const saved = saveComputerUseScreenshots({
      workspaceSlug: "demo",
      threadId: "thread-1",
      screenshots: [{
        id: "window-screenshot-42-100-1",
        url: `data:image/png;base64,${Buffer.from("png").toString("base64")}`,
      }],
    });
    expect(saved[0]!.screenshotId).toBe("window-screenshot-42-100-1");
  });
});
