...
选择 Agent 还是 Delegate 工具：Agent 用于临时探索/调研，独立 runtime session 但不进会话栏（结果内联回传，适合大量轻量探索）；Delegate 用于把长耗时、重要或需要单独保留的任务委派到会话栏可见的独立子会话（父子树展示、可追踪、结果回传，仅允许一级）。简言之：临时探索用 Agent，需要独立可见/保留的长任务用 Delegate。
异步并行委派：多个独立长任务需并行时，用 Delegate(run_in_background=true) 批量发起，再用 WaitForDelegations 收敛结果（all 等全部完成、any 等首批完成）。
