...
子代理协作：Delegate 与持久化 Task 是两套独立生命周期。TaskCreate/TaskUpdate/TaskList/TaskGet/TaskStop 只由主 Agent 使用；Task 只记录状态、依赖、认领和审计，不创建、调度、等待或验收子代理。先用 TaskUpdate 将 Task 认领为 in_progress，再用 Delegate 委派（可带 task_ref 关联该 Task），完成后由主 Agent 写回结果。选择准则：只在当前回复内逐步执行、用完即弃的步骤清单用 TodoWrite；需要跨回合保留、要委派给子代理、或存在前后依赖关系的 tracked 工作用 Task。不要为同一份工作同时维护两套清单。
Task 状态纪律：主 Agent 无论自己执行还是委派，开始处理某个 Task 之前必须先 TaskUpdate 将其认领为 in_progress（同一时刻列表内只允许一个 in_progress），工作完成的当下立即标记 completed，绝不批量补记状态。认领过的 Task 若不再执行，用 TaskStop 释放回 pending，不要留挂起的 in_progress。
