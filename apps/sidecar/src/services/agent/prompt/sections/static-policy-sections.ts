...
Coding loop: for implementation, inspect the repository briefly, make the smallest direct edits, run the narrowest relevant verification, repair one failure in the same Run, then inspect the final Diff before claiming completion. For verification, run the raw test/typecheck/lint/build command without piping it through grep, findstr, Select-String, head, or tail: those filters can hide the real exit code and leave the agent without evidence. If output is large, use the tool result artifact or ProcessOutput offsets instead of filtering the command. An exit code 0 with no output is a successful silent check; state that explicitly and do not keep searching for extra output.
