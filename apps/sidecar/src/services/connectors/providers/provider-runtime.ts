...
import { CastError, optionalRecord, optionalScalarString, optionalString, requiredString } from "../core/cast";
import { createGuardedFetch } from "../core/guarded-fetch";
import { readBoundedResponseBytes } from "../core/request";
...
export type ProviderActionName<TService extends string> = string;
...
export type ProviderActionHandlerSubset<TService extends string, THandler> = Partial<
  ProviderActionHandlers<TService, THandler>
>;

export type ProviderActionSources<TService extends string, TSource> = Record<
  string,
  TSource
>;
...
>(
  ...
  createHandler: (source: TSource, name: string) => THandler,
): ProviderActionHandlers<TService, THandler> {
  return Object.fromEntries(
    sources.map((source) => [source.name, createHandler(source, source.name as string)]),
 ...
 */
export function mapProviderActionNames<const TService extends string, THandler>(
  _service: TService,
  names: readonly string[],
  createHandler: (name: string) => THandler,
): ProviderActionHandlers<TService, THandler> {
  return Object.fromEntries(
    names.map((name) => [name, createHandler(name as string)]),
  ) as ProviderActionHandlers<TService, THandler>;
}
...
>(
  _service: TService,
  sources: TSources,
  createHandler: (name: string, source: TSources[string]) => THandler,
): ProviderActionHandlers<TService, THandler> {
  return Object.fromEntries(
    Object.entries(sources).map(([name, source]) => [
      name,
      createHandler(name as string, source as TSources[string]),
    ]),
  ) as ProviderActionHandlers<TService, THandler>;
}

/** Combine handler fragments whose completeness is checked by the final generated contract. */
export function combineProviderActionHandlers<const TService extends string, THandler>(
  _service: TService,
