...
export class PlanningTodoStore {
  ...
  create(input: PlanningTodoCreateInput): PlanningTodoMutationResult {
    ...
    const existing = this.#db.prepare(`SELECT * FROM planning_todo WHERE normalized_title = ? AND status = 'open' AND deleted_at IS NULL AND (workspace_id = ? OR (workspace_id IS NULL AND ? IS NULL)) LIMIT 1`).get(normalizedTitle, input.workspaceId ?? null, input.workspaceId ?? null) as TodoRow | undefined;
