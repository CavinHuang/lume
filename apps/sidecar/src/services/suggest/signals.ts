/**
 * Suggestion 信号提取 — 从用户消息中提取结构化信号
 *
 * 1:1 移植自 Proma `apps/electron/src/main/lib/suggest/signals.ts` (PR proma-ai/Proma#1409)。
 ...
 *
 * Lume 适配（相对 Proma 源）：
 * 1. 入参形状：Proma 接收 `string[]`，Lume 接收 `{role:"user"; content:string}[]`
 *    （brief 契约要求）。内部仍归约为 `string[]` 走 Proma 算法。
 * 2. NEGATIVE 暴露：Proma 命中"纯拒绝短句"后 `continue`（静默丢弃）；
 *    Lume 改为产出 `negative` 信号，供 engine 层（Task 4）做"最近一条拒绝词"门判断。
 * 3. detectRepeatIntents：在 intentKey 切片前剥离 WEAK_INTENT_KEYS 子串，
 *    使 "帮我跑测试" 与 "帮我跑一下测试" 归并为同一意图键 "跑测"
 *    （brief 契约测试要求；不破坏 Proma 既有 repeat/弱意图回归）。
 * 4. hasStrongSignal：Proma 接收 `string[]`；Lume 改为单条文本 `(text: string) => boolean`
 *    （brief 契约测试要求）。
 */

// ===== 信号模式表（verbatim from Proma） =====
...
export const WEAK_INTENT_KEYS: readonly string[] = [
  "看看",
  "一下",
  "这个",
  "那个",
...
/** NEGATIVE 拒绝信号（Lume 适配：暴露给 engine 层） */
...
/**
 * 从用户消息中提取建议信号。
 * @param messages 用户消息（按时间序，仅 user 角色）
 */
export function extractSignals(messages: readonly UserMessage[]): Signal[] {
  const userTexts = messages.map((m) => m.content);
  return extractSignalsFromTexts(userTexts);
}

/** 内部：基于纯文本数组的核心算法（1:1 移植 Proma + Lume 适配） */
function extractSignalsFromTexts(userMessages: string[]): Signal[] {
  const signals: Signal[] = [];

  for (let i = 0; i < userMessages.length; i++) {
    const text = userMessages[i] ?? "";

    // 明确拒绝信号：仅当整条消息就是拒绝（短句）时标记 negative，
    // 避免"不用管那个bug，以后写代码注意点"这类含拒绝词但主体是纠正的消息被过度抑制。
    // engine 层的"最后一条含拒绝词"门已兜底。
    const cleanText = text.replace(/[，。！？\s]/g, "");
    const isPureRejection = cleanText.length <= 12 && NEGATIVE_PATTERNS.some((re) => re.test(text));
    if (isPureRejection) {
      // Lume 适配：Proma 在此 `continue` 静默丢弃；Lume 暴露 negative 信号供 engine 决策。
      signals.push({
        kind: "negative",
        raw: text,
        messageIndex: i,
...
function detectRepeatIntents(userMessages: string[]): RepeatSignal[] {
  ...
  // Lume 适配：按长度降序，避免 "帮我" 先于 "帮我搞" 被部分剥离
  const weakKeysDesc = [...WEAK_INTENT_KEYS].sort((a, b) => b.length - a.length);
  const weakStripRe = new RegExp(weakKeysDesc.join("|"), "g");
  ...
  for (let i = 0; i < userMessages.length; i++) {
    ...
    let intentKey = intent.slice(0, 2);
    // 弱意图词（看看/一下/这个/那个）不构成可自动化操作的重复行为
    if (WEAK_INTENT_KEYS.includes(intentKey)) continue;
    if (/^(一下|这个|那个|帮我)$/.test(intentKey)) continue;

    // Lume 适配：剥离意图文本中的弱修饰词（如"一下"），再重新切片。
    // 使 "帮我跑一下测试" 与 "帮我跑测试" 归并为同一意图键 "跑测"。
    const stripped = intent.replace(weakStripRe, "").trim();
    if (stripped.length >= 2) {
      const strippedKey = stripped.slice(0, 2);
      // 剥离后再次过滤弱意图键
      if (!WEAK_INTENT_KEYS.includes(strippedKey) && !/^(一下|这个|那个|帮我)$/.test(strippedKey)) {
        intentKey = strippedKey;
      }
    }
...
export function isMeaningfulRule(rule: string): boolean {
  ...
  // 无意义残留词
  if (/^(这样|那样|再说|再聊|再说吧|而已|罢了|好了|算了|没事|这个|那个|一下)$/.test(trimmed))
    return false;
  return true;
}
