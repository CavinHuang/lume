...
import { createSuggestionHandlers as createSuggestionHandlersImpl } from "./suggestion-handlers";

/**
 * Handler 接线测试：注入 store/service fake，验证每个 channel 调对函数 + 透传参数。
...
const createSuggestionHandlers = (
  context: Parameters<typeof createSuggestionHandlersImpl>[0],
) => createSuggestionHandlersImpl(
  context,
  { ...storeMocks, ...serviceMocks },
);
