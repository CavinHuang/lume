...
async function run() {
  ...
  try {
    ...
    // #684 把 CONFIG_VERSION 升到 2（PR#729 补）：首建默认配置落盘即 v2。
    // 断言取 >= 数值比较（#531 复审），上游再升版本不会 brick smoke；
    // writeSmokeLumeConfig 的 version:1 夹具仍刻意保留，用于覆盖 v1→v2 迁移读路径。
    assert(Number(initialLumeConfig?.version) >= 2, "lume-config:get-effective unavailable");
    assert(
