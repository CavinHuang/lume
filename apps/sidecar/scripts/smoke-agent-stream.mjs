...
async function run() {
  const configHome = mkdtempSync(join(tmpdir(), "lume-sidecar-agent-smoke-"));
  let sidecar = null;
  ...
  try {
    ...
    assert(typeof errorEvent?.error === "string", "missing stream error payload");
    assert(errorEvent.error.includes("未找到可用渠道"), "unexpected stream error message");

    console.log("SMOKE_AGENT_STREAM_OK");
  } finally {
    if (sidecar) {
      try {
        await sidecar.close();
...
run().catch((error) => {
  console.error("SMOKE_AGENT_STREAM_FAIL", error instanceof Error ? error.message : String(error));
  process.exit(1);
});
