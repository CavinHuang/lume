...
function createSidecarProcess(configHome) {
  ...
  env.LUME_PI_AGENT_MOCK_DELAY_MS = "3000";
  env.LUME_PI_AGENT_MOCK_TEXT = "smoke-new-runtime-stop";
...
async function run() {
  const configHome = mkdtempSync(join(tmpdir(), "lume-sidecar-agent-new-runtime-stop-"));
  let sidecar = null;
  ...
  try {
    ...
    const channel = await sidecar.call("channel:create", {
      name: "smoke-anthropic-new-runtime-stop",
      provider: "anthropic",
...
run().catch((error) => {
  console.error("SMOKE_AGENT_NEW_RUNTIME_STOP_FAIL", error instanceof Error ? error.message : String(error));
  process.exit(1);
});
