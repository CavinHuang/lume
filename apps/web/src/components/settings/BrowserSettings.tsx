...
import type { BrowserSettings as BrowserSettingsValue } from '@lume/shared'
...
export function BrowserSettings({ onOpenSkills: _onOpenSkills }: { onOpenSkills?: () => void }) {
  ...
  return <div className="space-y-3">
    ...
    <section className="lume-panel-padded"><div className="mb-3 flex items-start gap-3"><ShieldCheck className="mt-1 text-[var(--brand)]" size={18} /><SectionTitle title="权限" description="网站权限和完整 origin 覆盖；Agent task partition 默认拒绝敏感权限。" /></div>
      ...
      <div className="border-t border-[var(--border)] py-3"><div className="text-[13px] font-medium text-[var(--text-2)]">站点覆盖（完整 origin）</div><div className="mt-2 flex gap-2"><Input value={overrideOrigin} onChange={(event) => setOverrideOrigin(event.target.value)} placeholder="https://example.com" /><Select value={overrideDecision} onValueChange={(value) => setOverrideDecision(value as typeof overrideDecision)}><SelectTrigger className="w-24"><SelectValue /></SelectTrigger><SelectContent><SelectItem value="ask">询问</SelectItem><SelectItem value="allow">允许</SelectItem><SelectItem value="deny">拒绝</SelectItem></SelectContent></Select><Button variant="outline" size="sm" disabled={!normalizedOverrideOrigin} onClick={() => { if (normalizedOverrideOrigin) void save({ siteOverrides: { ...settings.siteOverrides, [normalizedOverrideOrigin]: overrideDecision } }); setOverrideOrigin('') }}>保存</Button></div></div>
      {Object.entries(settings.siteOverrides).map(([origin, decision]) => <div key={origin} className="flex items-center justify-between gap-3 border-t border-[var(--border)] py-3"><div className="min-w-0"><div className="truncate text-[13px] text-[var(--text-2)]">{origin}</div><div className="text-[12px] text-[var(--text-3)]">{decision === 'allow' ? '允许' : decision === 'deny' ? '拒绝' : '询问'}</div></div><Button variant="ghost" size="sm" onClick={() => { const next = { ...settings.siteOverrides }; delete next[origin]; void save({ siteOverrides: next }) }}>移除</Button></div>)}
    </section>
    <section className="lume-panel-padded"><SectionTitle title="开发者模式" description="高风险能力只对新建的隔离空白 Profile 生效。" />
      ...
      <SettingRow title="高级 CDP 访问权限" description="仅隔离临时会话和 allowlist 方法；禁止 Runtime、Network、Storage 和跨 target。"><Switch checked={settings.advancedCdpEnabled} onCheckedChange={(checked) => void save({ advancedCdpEnabled: checked })} /></SettingRow>
    </section>
