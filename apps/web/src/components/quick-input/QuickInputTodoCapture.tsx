...
export function QuickInputTodoCapture({ workspaces, workspaceId, initialTitle = '', onSaved }: { workspaces: AgentWorkspace[]; workspaceId: string | null; initialTitle?: string; onSaved: () => void }) {
  ...
  const save = async () => { if (!title.trim()) return; setBusy(true); try { await createPlanningTodo({ title: title.trim(), priority, workspaceId: selectedWorkspaceId === 'unassigned' ? undefined : selectedWorkspaceId, ...(dueDate ? { dueDate } : {}), ...(dueAt ? { dueAt: new Date(dueAt).getTime(), dueTimezone: Intl.DateTimeFormat().resolvedOptions().timeZone } : {}) }); setTitle(''); onSaved() } catch (error) { toast.error(error instanceof Error ? error.message : '待办保存失败') } finally { setBusy(false) } }
