/**
 * Extend UI — CSV Viewer（shadcn registry: @extend/csv-viewer）
 *
 * 导出：具名 `CsvViewer`（function 组件），无 default 导出。
 * 文件源 prop：**`data?: string`**（注意：是文件**内容字符串**，不是 URL；调用方需先 fetch+text() 后传入）。
 *            无 `src` prop，无文件名/上传支持；上传由本组件内部 input 处理但仅本地解析。
 *
 * 关键 props（极简）：
 *   - className?: string
 *   - data?: string（CSV/TSV 文本内容；Papa.parse 自动识别分隔符，header 模式）
 *   - search?: boolean（默认 false，控制工具栏搜索控件）
 *
 * 暗色模式：内部 useIsDarkTheme() 自动检测（基于 prefers-color-scheme + 容器观察），不接受外部 prop。
 *
 * 外部依赖（不在 @/components/ui）：@glideapps/glide-data-grid（含 dist/index.css 导入）、papaparse、@hugeicons/{core-free-icons,react}。
 *
 * @/components/ui primitive：button、dropdown-menu、input、popover、select、separator、spinner、tooltip
 *   （全部存在于 Lume；spinner.tsx 由本次 shadcn add 一并生成）
 *   未使用：scroll-area、tabs
 * @/components/extend：无
 * @/lib：utils
 */
