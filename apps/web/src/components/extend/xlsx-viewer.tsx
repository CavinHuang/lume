/**
 * Extend UI — XLSX Viewer（shadcn registry: @extend/xlsx-viewer）
 *
 * 导出：具名 `XlsxViewerPreview`（function 组件），无 default 导出。
 *      另有具名导出 WorkbookTableHeaderMenu、WorkbookSheetTabs、XlsxWorkbookSurface、renderXlsxScroller（内部子组件，一般不直接用）。
 * 文件源 prop：`src?: string`（URL；上传走 showUpload + 内部 <input type=file>，accept=.xlsx/.xls，最大 50MB）。
 *
 * 关键 props（注意 isDark / onIsDarkChange 为必填）：
 *   - className?: string
 *   - fileName?: string
 *   - src?: string
 *   - isDark: boolean（**必填**）
 *   - onIsDarkChange: (isDark: boolean) => void（**必填**）
 *   - showToolbar? / showDownload? / showUpload?（默认 true）
 *   - toolbarActions?: React.ReactNode
 *
 * 外部依赖（不在 @/components/ui）：@extend-ai/react-xlsx、@hugeicons/{core-free-icons,react}。
 *
 * @/components/ui primitive：button、dropdown-menu、input、popover、scroll-area、select、separator、spinner、tabs、tooltip
 *   （全部存在于 Lume；spinner.tsx 由本次 shadcn add 一并生成）
 * @/components/extend：无（自身无 sidebar 引用）
 * @/lib：utils
 */
"use client"
...
function WorkbookFileActionsMenu({
  ...
  return (
    <DropdownMenu>
      <DropdownMenuTrigger render={<Button type="button" variant="ghost" size="icon-sm" aria-label="Open workbook actions" />}><HugeiconsIcon icon={MoreHorizontalIcon} className="size-4" /></DropdownMenuTrigger>
      <DropdownMenuContent
        align="end"
...
export function WorkbookTableHeaderMenu({
  ...
  return (
    <DropdownMenu open={open} onOpenChange={setOpen}>
      <DropdownMenuTrigger render={<Button {...triggerProps} type="button" variant="ghost" size="icon-sm" className={cn("size-6 rounded-sm", triggerProps.className)} aria-label="Column menu" />}>{triggerIcon ? (
                      triggerIcon
                    ) : (
  ...
  return (
    <Popover>
      <ToolbarTooltip label="Search workbook">
        <PopoverTrigger render={<Button type="button" variant="ghost" size="icon-sm" aria-label="Search workbook" disabled={controlsDisabled} />}><HugeiconsIcon icon={Search01Icon} className="size-4" /></PopoverTrigger>
      </ToolbarTooltip>
