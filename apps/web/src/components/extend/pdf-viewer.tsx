/**
 * Extend UI — PDF Viewer（shadcn registry: @extend/pdf-viewer）
 *
 * 导出：具名 `PDFViewer`（React.forwardRef<PDFViewerHandle, PDFViewerProps>），无 default 导出。
 * 文件源 prop：`src?: string`（URL 或 blob: URL；上传走 showUpload + 内部 handleUploadFile + onPdfUpload 回调）。
 *
 * 关键 props：
 *   - className?: string
 *   - defaultZoom?: number（默认 1；ZOOM_OPTIONS=[0.1,0.25,0.5,0.75,1,1.25,1.5,2]）
 *   - fileName?: string
 *   - src?: string
 *   - showToolbar? / showDownload? / showUpload? / showRotateControls?（均默认 true）
 *   - toolbarActions?: React.ReactNode
 *   - resolveScrollAreaViewport?: (container: HTMLDivElement) => HTMLDivElement | null
 *   - pageClassName?: (pageNumber: number) => string | undefined
 *   - renderPageOverlay?: (props: PDFViewerPageOverlayProps) => React.ReactNode
 *   - onActivePageChange? / onDocumentLoadSuccess? / onPdfUpload?
 *   - onPagePointerDown? / Move? / Up? / Cancel?
 *   - ref 句柄：PDFViewerHandle { scrollToPage, scrollToPageArea, getViewportElement }
 *
 * 外部依赖（不在 @/components/ui）：@embedpdf/*（13 个 plugin/core/models/engines 包）、pdf-lib、@hugeicons/{core-free-icons,react}。
 *   注：PDF 渲染走 @embedpdf 引擎（非 react-pdf）；engine 通过 @/lib/pdf-thumbnail-utils 的 loadSharedPdfEngine() 初始化。
 *
 * @/components/ui primitive：button、dropdown-menu、input、popover、scroll-area、select、separator、spinner、tooltip
 *   （全部存在于 Lume；spinner.tsx 由本次 shadcn add 一并生成）
 * @/components/extend：document-viewer-sidebar
 * @/lib：pdf-thumbnail-utils（本次新增）、utils
 */
