/**
 * Extend UI — PPTX Viewer（shadcn registry: @extend/pptx-viewer）
 *
 * 导出：具名 `PptxViewerPreview`（function 组件），无 default 导出。
 *      另有具名导出 PptxViewerPreviewProps 类型。
 * 文件源 prop：`src?: string`（URL；上传走 showUpload + 内部 <input type=file>，accept=.pptx/.ppt）。
 *
 * 关键 props：
 *   - className?: string
 *   - defaultZoom?: number（默认 100；范围 10-400；ZOOM_OPTIONS=[25,50,75,100,125,150,175,200,300,400]）
 *   - fileName?: string
 *   - src?: string
 *   - initialSlide?: number（默认 1，1-based）
 *   - defaultThumbnailSidebarOpen?: boolean（默认 false）
 *   - showToolbar? / showDownload? / showUpload?（默认 true）
 *   - toolbarActions?: React.ReactNode
 *
 * 外部依赖（不在 @/components/ui）：@extend-ai/react-pptx（含 styles.css 导入）、@tanstack/react-virtual、@hugeicons/{core-free-icons,react}。
 *
 * @/components/ui primitive：button、dropdown-menu、input、scroll-area、select、separator、spinner、tooltip
 *   （全部存在于 Lume；spinner.tsx 由本次 shadcn add 一并生成）
 * @/components/extend：document-viewer-sidebar、file-thumbnail
 * @/lib：utils
 */
"use client"
...
function PptxFileActionsMenu({
  ...
  return (
    <DropdownMenu>
      <DropdownMenuTrigger render={<Button type="button" variant="ghost" size="icon-sm" aria-label="Open PowerPoint actions" />}><HugeiconsIcon icon={MoreHorizontalIcon} className="size-4" /></DropdownMenuTrigger>
