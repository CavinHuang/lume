/**
 * Extend UI — DOCX Viewer（shadcn registry: @extend/docx-viewer）
 *
 * 导出：具名 `DocxViewerPreview`（function 组件），无 default 导出。
 * 文件源 prop：`src?: string`（URL；上传走 showUpload + 内部 <input type=file>，accept=.doc/.docx）。
 *
 * 关键 props（注意 isDark / onIsDarkChange 为必填）：
 *   - className?: string
 *   - defaultZoom?: number（默认 50；ZOOM_OPTIONS=[10,25,50,75,100,125,150,175,200,400]，单位百分比）
 *   - fileName?: string
 *   - src?: string
 *   - isDark: boolean（**必填**）
 ...
 *
 * 外部依赖（不在 @/components/ui）：@extend-ai/react-docx、@tanstack/react-virtual、@hugeicons/{core-free-icons,react}。
 *
 ...
 * @/components/extend：document-viewer-sidebar、docx-annotation-card、file-thumbnail
 * @/lib：utils
 */
"use client"
