 ...
 *    接入真实任务流时用 `phase` 覆盖演示时序（缺省回落到 useTick 编排）
 */
export type TaskRow = {
  key: string;
  label: string;
...
export default function TaskRows({
  ...
  const phaseOf = (row: TaskRow): "pending" | "failed" | "done" => row.phase ?? row2;
