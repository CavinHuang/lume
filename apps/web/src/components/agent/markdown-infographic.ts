...
export function getInfographicCodeFromPreNode(value: unknown): string | null {
  const codeNode = getCodeNodeFromPre(value)
  if (!codeNode || getFenceLanguage(codeNode) !== 'infographic') return null
  return getNodeText(codeNode).replace(/\r?\n$/, '')
}

export function isInfographicPreStreaming(value: unknown): boolean {
  const codeNode = getCodeNodeFromPre(value)
