...
type WorkspaceSelectorProps = {
  ...
  onWorkspaceChange?: (workspaces: AgentWorkspace[]) => void;
};
...
export function WorkspaceSelector({
  ...
  const currentWorkspace = workspaces.find((w) => w.id === value);
  ...
  const handleCreate = async (): Promise<void> => {
    ...
    try {
      ...
      // 通知父组件更新工作区列表
      onWorkspaceChange?.([...workspaces, workspace]);
      onChange(workspace.id);
      setCreating(false);
      setOpen(false);
    } catch (error) {
  ...
  const handleRename = async (): Promise<void> => {
    ...
    try {
      const updated = await updateAgentWorkspace(editingId, trimmed);
      // 通知父组件更新工作区列表
      onWorkspaceChange?.(workspaces.map((w) => (w.id === updated.id ? updated : w)));
    } catch (error) {
  ...
  const handleConfirmDelete = async (): Promise<void> => {
    ...
    try {
      ...
      // 通知父组件更新工作区列表
      onWorkspaceChange?.(remaining);

      // 如果删除的是当前工作区，切换到第一个剩余的
      if (deleteTargetId === value && remaining.length > 0) {
        const firstRemaining = remaining[0];
        if (firstRemaining) onChange(firstRemaining.id);
