...
const WORKSPACE_NM = resolve(REPO_ROOT, "node_modules", ".bun");
...
try { spawnSync("rm", ["-rf", tmpDir], { stdio: "inherit" }); } catch {}
...
for (const pkg of packagesToCopy) {
  ...
  console.error(`[sidecar-binary] staged node_modules/.bun/${pkg} -> _nm-tmp/${pkg}`);
}

// Zip the staged node_modules for Tauri bundling
console.error(`[sidecar-binary] creating zip: ${NODE_MODULES_ZIP}`);
const zipResult = spawnSync("zip", ["-r", "-q", NODE_MODULES_ZIP, "."], {
  cwd: tmpDir,
  stdio: "inherit",
});
