...
- ✅ 已完成：`nano_banana` 编辑语义策略（自动启用参考图、`16:9/9:16/4:3/3:4` 画幅与 `1K/2K/4K` 分辨率推断）。
- ✅ 已完成：`nano_banana` 风格/约束模板化提示增强（`Style hints` + `Constraints` 双段落拼接）。
- ✅ 已完成：`nano_banana` 深度提示词自动优化（跨轮意图记忆 `Intent memory`、约束冲突自动裁剪、主风格显式覆盖历史）。
