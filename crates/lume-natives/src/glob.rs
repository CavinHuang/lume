...
fn filter_entries(
	...
	for entry in entries {
		...
		let effective_file_type = match apply_file_type_filter(entry, config) {
			Some(ft) => ft,
			None => continue,
		};
...
fn collect_sorted_matches_uncached(
	...
	for entry in builder.build() {
		...
		let matched_entry = match fs_cache::collect_entry(
			&config.root,
			&entry,
		...
		let effective_file_type = match apply_file_type_filter(&matched_entry, config) {
			Some(ft) => ft,
			None => continue,
		};

		let mut matched_entry = matched_entry;
		matched_entry.file_type = effective_file_type;
		...
		if push_bounded_match(&mut top_matches, matched_entry, config.max_results) {
			if let Some((callback, payload)) = streamable {
...
fn run_glob(
	...
	if config.sort_by_mtime {
		...
		if !streams_bounded_sorted_partials {
			if let Some(callback) = on_match {
