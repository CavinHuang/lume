...
impl Drop for WorkspaceVisitor<'_> {
	fn drop(&mut self) {
		if !self.entries.is_empty() {
			...
			self.shared_entries.lock().unwrap().push(entries);
		}
		if !self.agents_md_files.is_empty() {
			let agents_md_files = std::mem::take(&mut self.agents_md_files);
			self.shared_agents_md_files.lock().unwrap().push(agents_md_files);
		}
	}
}
...
impl ParallelVisitor for WorkspaceVisitor<'_> {
	fn visit(&mut self, entry: std::result::Result<DirEntry, ignore::Error>) -> WalkState {
		...
		if entry_depth <= self.config.max_depth {
			if let Some(entry) = glob_match_from_entry(&self.config.root, &entry) {
...
fn run_list_workspace(
	...
	let walk_error = error.lock().unwrap().take();
	if let Some(error) = walk_error {
		return Err(Error::from_reason(error));
	}

	let mut entries: Vec<GlobMatch> =
		shared_entries.lock().unwrap().drain(..).flatten().collect();
	entries.extend(root_entries);
	sort_dedup_entries(&mut entries);

	let mut agents_md_files: Vec<String> =
		shared_agents_md_files.lock().unwrap().drain(..).flatten().collect();
...
mod tests {
	...
	impl TempDirGuard {
		fn new() -> Self {
			...
			let path = std::env::temp_dir().join(format!("lume-workspace-test-{timestamp}-{counter}"));
	...
	fn scan(root: &Path, max_depth: usize, collect_agents_md: bool) -> ListWorkspaceResult {
		run_list_workspace(
			WorkspaceConfig {
				root: root.to_path_buf(),
				max_depth,
				walk_max_depth: if collect_agents_md {
					max_depth.max(AGENTS_MD_MAX_DEPTH)
				} else {
					max_depth
				},
	...
	fn skips_git_and_node_modules() {
		...
		write_file(&root.path().join(".git/config"), "git");
		write_file(&root.path().join("node_modules/pkg/index.js"), "pkg");
		write_file(&root.path().join("src/main.ts"), "main");

		let result = scan(root.path(), 3, false);
	...
	#[test]
	fn max_depth_limits_entries_but_still_collects_agents_md() {
		let root = TempDirGuard::new();
		write_file(&root.path().join("docs/AGENTS.md"), "rules");
		write_file(&root.path().join("docs/nested/file.ts"), "nested");

		let result = scan(root.path(), 1, true);
		let paths: Vec<&str> = result.entries.iter().map(|entry| entry.path.as_str()).collect();

		assert!(paths.contains(&"docs"));
		assert!(!paths.contains(&"docs/AGENTS.md"));
		assert!(!paths.contains(&"docs/nested/file.ts"));
		assert_eq!(result.agents_md_files, vec!["docs/AGENTS.md"]);
	}
}
